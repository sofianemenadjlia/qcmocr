...
sub version_upgrade {
    ...
    if ( $old_version == 0 ) {
        ...
        $self->sql_do( "CREATE TABLE IF NOT EXISTS "
              ...
              . " (student INTEGER, copy INTEGER, manual TEXT, auto TEXT, confidence REAL, PRIMARY KEY (student,copy))"
        );
...
# populate_from_xml_OCR est inspiré de populate_from_xml.
# Cette version lis un xml nommé associationOCR.xml dans
# le dossier cr, elle ajoute les données dans la base de données
# et compte le nombre d'associations trouvé

sub populate_from_xml_OCR {
    my ($self, $student_csv) = @_;
    my $assoc_file = $self->{data}->directory;
    $assoc_file =~ s:/[^/]+/?$:/cr/associationOCR.xml:;
    return if ( !-f $assoc_file );
    
    $self->progression( 'begin',
        __ "Lecture du XML généré en cours..." );
    ...
    my @possibleNumCopie = $self->getPossibleNumCopie;

    my @possibleIdEtudiant = $self->getPossibleIdEtudiant($student_csv, $a->{liste_key});
    ...
    for my $student (@s) {
        my $s = $a->{copie}->{$student};
        if($self->isValideData($s->{id}, $s->{num}, @possibleNumCopie) && (grep { $_ eq $s->{num} } @possibleIdEtudiant))
        {
            $compteur += 1;
            my $n = $self->statement('insert')->execute( $s->{id}, 0, undef, $s->{num}, $s->{confidence} );
            if ( $n <= 0 ) {
                $self->statement('setAuto')->execute( $s->{num}, $s->{confidence}, $s->{id}, 0 );
            }
        }
        else
        {
...
# isValideDate retourne true si la donnée
# recu lu dans le xml est valide, c'est à dire
# si le numéro de copie et le l'identifiant de
# l'étudiant existent

sub isValideData {
    my ( $self, $numCopie, $idEtud, @possibleNumCopie ) = @_;

    #On vérifie que le numéro de la copie est valide
    my $estValideNumCopie = (grep { $_ eq $numCopie } @possibleNumCopie);

    #Pour une raison obscure, je ne peux pas verifier ici si l'identifiant
    #de l'etudiant est valide, donc je fais ca dans le if au dessus direct

    return ($estValideNumCopie);
}

sub getPossibleNumCopie {
    my ( $self ) = @_;

    my @files_matching_name;
    my $cr_directory = substr($self->{data}->directory, 0, -4) . "cr";

    opendir(my $dh, $cr_directory) or die "Impossible d'ouvrir le répertoire $cr_directory : $!";
    ...
    while (my $filename = readdir($dh)) {
        ...
        # On ajoute ceux qui commencent par "name"
        if ($filename =~ /^name/) {
            push @files_matching_name, (substr($filename, 5, -4));        
        }
    }

    closedir($dh);

    return @files_matching_name;
}

sub getPossibleIdEtudiant
{
    my ( $self, $csv_file, $key ) = @_;

    my @possibleIdEtudiant;

    # Créer un objet Text::CSV
    my $csv = Text::CSV->new({ binary => 1, auto_diag => 1, sep_char => ';' }) or die "Impossible d'utiliser Text::CSV: " . Text::CSV->error_diag();

    # Ouvrir le fichier CSV
    open(my $fh, "<", $csv_file) or die "Impossible d'ouvrir le fichier $csv_file: $!";

    # Lire les en-têtes pour obtenir les noms des colonnes
    ...
    while (my $row = $csv->getline($fh)) {
        my $value = $row->[$index];
        push @possibleIdEtudiant, "$value";
    }

    # Fermer le fichier CSV
    close($fh);

    return @possibleIdEtudiant;

}

# isValideId retourne true si l'id choisi
# par l'utilisateur correspond à l'id
# généré par OCR dans le fichier XML
...
sub isValideId {
    ...
    my $a = XMLin( $i );
    $i->close();
    return ($id eq $a->{liste_key});

}

# isXmlGenerated retourne true si le xml
# que l'on souhaite lire existe bien
...
sub define_statements {
    ...
    $self->{statements} = {
        NEWAssoc => {
            sql => "INSERT INTO $at"
              . " (student,copy,manual,auto,confidence) VALUES (?,?,?,?,?)"
        },
        insert => {
        ...
        setAuto =>
          { sql => "UPDATE $at" . " SET auto=?, confidence=? WHERE student=? AND copy=?" },
        getReal => {
                sql => "SELECT CASE"
        ...
        realCounts => {
            sql =>
" SELECT t.student AS student,t.copy AS copy,t.real AS real,t.manual AS manual,t.auto AS auto, t.confidence AS confidence, c.n AS n"
              . " FROM"
              . " ( SELECT CASE"
              . "   WHEN manual IS NOT NULL THEN manual"
              . "   ELSE auto END AS real,student,copy,manual,auto,confidence FROM $at"
              . " ) AS t,"
              . " ( SELECT CASE"
              . "   WHEN manual IS NOT NULL THEN manual"
              . "   ELSE auto END AS real,"
        ...
        realBack => {
              ...
              . " WHEN manual IS NOT NULL THEN manual"
              . " ELSE auto END AS real, student, copy, confidence"
              . " FROM $at"
              . " ) WHERE real=?||''"
        },
        realBackInt => {
...
sub get_auto {
    my ( $self, $student, $copy ) = @_;
    my $state = $self->state($student, $copy);
    if($state == 2)
    {
        return (($self->sql_single( $self->statement('getAuto'), $student, $copy )) . " (d)" ) ;
    }
    else
    {
        return ($self->sql_single( $self->statement('getAuto'), $student, $copy )) ;
    }
...
sub set_manual {
    ...
    my $n =
      $self->statement('insert')->execute( $student, $copy, $manual, undef, 100.0 );
    if ( $n <= 0 ) {
...
sub set_auto {
    my ( $self, $student, $copy, $auto, $confidence ) = @_;
    my $n =
      $self->statement('insert')->execute( $student, $copy, undef, $auto, $confidence );
    if ( $n <= 0 ) {
        $self->statement('setAuto')->execute( $auto, $confidence, $student, $copy );
    }
}
...
sub counts_hash {
    ...
    {
        #On appelle makeColor avec notre taux de confiance pour générer la couleur de la case
        # my $gradientColor = $self->makeColor($l->{confidence});
        ...
        $r->{ studentids_string( $l->{student}, $l->{copy} ) } = {
            ...
            auto => (
                $etat == 2 && defined($l->{manual}) && $l->{manual} eq 'NULL' ? ($l->{auto} . " (d)") : $l->{auto}
            ),
            ...
            color => (
                ...
                : $etat == 1 ? ( $l->{manual} ? 'lightgreen' : $self->makeColor($l->{confidence}) )
                :              'salmon'
            ),
...
# makeColor($confidence) génére une couleur rgb en fonction de la valeur
# de confidence, qui est une valeur entre 0 et 100

sub makeColor {
    my ( $self, $confidence ) = @_;
    my $seuil = 98;
    if($confidence > 100 || $confidence < 0) #Un cas qui ne devrait pas arriver en théorie
    {
        #On affiche en rouge pour bien voir qu'il y a un problème
        return "#ff0000";
    }
    elsif($confidence > $seuil) #On est au dessus du seuil donc on affiche directement en vert
    {
        return "lightgreen";
    }
    else
    {
        #On affiche une couleur qui varie entre rouge, orange et jaune
        my $green = sprintf("%02x", int((255 * $confidence / $seuil)));
        my $gradientColor = "#" . "ff" . "$green" . "00";
