...
sub efface_manuel {
    ...
    if ( $i >= 0 ) {
        ...
        while ($ok) {
            my @sc1 =
                $self->{copies_store}->get( $iter, COPIES_STUDENT, COPIES_COPY );
            $self->maj_contenu_liste( $i, $iter, @sc1 );
            $ok = $self->{copies_store}->iter_next($iter);
        }
...
# Cancels auto association of current sheet.
#
# {OUT:}
...
sub efface_auto {
    ...
    if ( $i >= 0 ) {
        ...
        $self->{capture}->outdate_annotated_copy(@sc);
        $self->{assoc}->set_auto( @sc, undef , undef);
...
# Cancels all associations.
#
# {OUT:}
sub reset_action {
    my ($self) = @_;

    $self->{assoc}->begin_transaction('RESET');

    my $nb_images = scalar @{ $self->{images} };
    ...
    $self->{assoc}->end_transaction('RESET');
}
